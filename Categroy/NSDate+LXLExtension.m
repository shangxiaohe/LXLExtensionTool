//
//  NSDate+LXLExtension.m
//  LXLExtension
//
//  Created by lixianliang on 2021/7/19.
//

#import "NSDate+LXLExtension.h"
...
@implementation NSDate (LXLExtension)
...
- (BOOL)isSameDayToDate:(NSDate*)date
{
    ...
    NSDateComponents *componentSelf = [calendar components:unit fromDate:self];
    NSDateComponents *componentDate = [calendar components:unit fromDate:date];
    
    return ([componentSelf year] == [componentDate year]
            && [componentSelf month] == [componentDate month]
            && [componentSelf day] == [componentDate day]);
}

- (NSInteger)daysSinceReferenceDate:(NSDate*)date
{
    if (date == nil) {
        return NSNotFound;
    }
    ...
    components = [calendar components:unitFlag fromDate:[NSDate dateWithTimeIntervalSince1970:0] toDate:self options:0];
    NSInteger oldDayCount = [components day];
    
    return oldDayCount - newDayCount;
}

- (NSString *)timeIntervalStringSinceReferenceDate:(NSDate*)date {
    NSCalendar *calendar = [NSCalendar currentCalendar];
    ...
    if ([components year] != 0) {
        ...
        return [NSString stringWithFormat:@"%ld周前", (long)[components weekOfYear]];
    }
...
+ (NSString *)dateWithNSInteger:(NSInteger)integer {
    NSInteger num = integer;
    ...
    NSDate *confromTimesp = [NSDate dateWithTimeIntervalSince1970:num];NSString *confromTimespStr = [formatter stringFromDate:confromTimesp];
...
+ (NSDate *)getCurrentTimeZoneWithNowDate:(NSDate *)date {
